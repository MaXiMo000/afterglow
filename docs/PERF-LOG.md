# Performance log

Budgets: `docs/PLAN.md` section 6. Record every measurement here in the same PR as the change that affects it.
Numbers must come from a named device and tool; anything estimated or not measured says so.

| Date | Milestone | Device / browser | Metric | Result | Budget | Tool | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 2026-09-24 | A0 | n/a (build output) | Initial JS gzip | 0.12 KB JS + 0.43 KB CSS | <= 400 KB | `vite build` | Placeholder page only; no renderer yet |
| 2026-09-25 | A1 | Windows 11 laptop, local git 2.55 | Analysis wall time, fastapi/typer (1,762 commits, 775 files) | ~4 s | 90 s cap | `python -m worker` | Includes both clones over the network |
| 2026-09-25 | A1 | same | Analysis wall time, fastapi/fastapi (7,713 commits, 3,139 files) | ~7 s | 90 s cap | `python -m worker` | |
| 2026-09-25 | A1 | n/a | Result size, fastapi/fastapi (3,139 files) | 606 KB raw, 55 KB gzip | <= 1.5 MB gzip at 50k files | gzip -6 | Linear extrapolation to 50k files is ~0.9 MB gzip as JSON; the binary format decision is re-measured in A2 |
| 2026-09-25 | A2 | Docker Desktop, full stack | Result on the wire, fastapi/fastapi (3,139 files) | 62.5 KB gzip | <= 1.5 MB gzip at 50k files | curl via Caddy | JSON kept; binary format dropped (PLAN section 3) |
| 2026-09-25 | A2 | same | POST -> done, fastapi/fastapi, cold | ~9 s | 90 s cap | curl + SSE | Includes queue pickup (<= 1 s poll) |
| 2026-09-25 | A3 | n/a (build output) | Initial JS gzip | 20.6 KB JS + 3.4 KB CSS (+ fonts ~110 KB woff2, cached) | <= 400 KB | `vite build` | Renderer included; no Three.js |
| 2026-09-25 | A3 | Windows laptop, headless Chromium (ANGLE d3d11, real GPU), 1440x900 | Frame rate, fastapi/typer city, cinematic | 60 fps (vsync-capped) | 60 fps | rAF counter over 120 frames | Not a phone. Not a sustained profile |
| 2026-09-25 | A3 | same, local stack | Hero ready incl. demo city (620 KB JSON) | 2.5 s | first frame <= 2.5 s on 4G | Playwright timing | Local network, desktop CPU: says nothing about 4G/mid-range Android |
| 2026-09-25 | A4 | n/a (build output) | Initial JS gzip | 30.8 KB JS (incl. Lenis) + 4.3 KB CSS | <= 400 KB | `vite build` | |
| 2026-09-25 | A4 | by construction | Camera latency to smoothed scroll | 0 frames | <= 1 frame | code: Lenis driven from our rAF, pose read in the same frame | Smoothing itself: lerp 0.14/frame, ~95% settled in ~20 frames (~330 ms at 60 Hz); **over the 150 ms target** in PLAN section 6 for large jumps. Tune in A7 with real devices |
| 2026-09-25 | A4 | unit test | Reverse-scroll symmetry | pose(p) identical for 201 samples forward vs backward | exact | vitest `rig.test.ts` | Pure function of p; handheld noise is additive and time-based, outside the rig |
| 2026-09-25 | A5 | n/a (build output) | Initial JS gzip | 39.7 KB JS + ~5 KB CSS | <= 400 KB | `vite build` | All explore tools included |
| 2026-09-25 | A5 | unit test | Zoom-to-cursor error | < 1.5 px at 1200x800 after a 0.7x zoom | point stays under cursor | vitest `camera.test.ts` | |
| 2026-09-25 | A6 | n/a (build output) | Initial JS gzip | 41.9 KB JS + ~6 KB CSS | <= 400 KB | `vite build` | All effects included |
| 2026-09-25 | A6 | Windows laptop, headless Chromium (ANGLE d3d11) | Story + city, cinematic, with DOF/flare/rays | no console errors; frame rate **not re-measured** | 60 fps | Playwright walkthrough | Extra passes: half-res DOF blur (3-5 passes), 20-tap rays, 1 ripple draw of N instances. Needs real-device profiling in A7 |
| 2026-09-25 | A6 | same, 3 parallel software-GPU pages | Long e2e walkthrough under stress | page crashes on both A5 and A6 builds | n/a | `--repeat-each=6 --workers=3` | Local machine overload, not a regression; CI uses 2 workers |
| 2026-09-26 | A7 | n/a (build output) | Initial JS gzip | 42.4 KB JS + ~6 KB CSS; 7 woff2 fonts 128 KB (preloaded) | <= 400 KB | `vite build` | |
| 2026-09-26 | A7 | Windows laptop, Chromium (Playwright), real GPU via ANGLE d3d11, 1440x900, 4G emulated (150 ms RTT, 1.6 Mbps) | Time to first rendered frame (first WebGL draw) | 2.0-2.3 s at 1x CPU; 2.6-3.4 s at 4-6x CPU throttle | <= 2.5 s on mid-range Android, 4G | `scripts/profile.mjs frames` | **Not a phone.** CPU throttle is a stand-in; mid-range Android not measured |
| 2026-09-26 | A7 | same, 1x CPU | Frame time, story and city, all tiers (pallets/flask) | p50 16.7 ms, p95 16.7 ms, 0-1% of frames over 16.7 ms | 60 fps desktop | `scripts/profile.mjs frames` | |
| 2026-09-26 | A7 | same, 4x CPU throttle | Frame time | p50 16.7 ms; p95 33-67 ms; 6-36% of frames over 16.7 ms | >= 30 fps low-end phone | same | After the dynamic-resolution fix (city was ~20% over before). CDP throttling slows only the page's main thread, not the GPU |
| 2026-09-26 | A7 | same, 6x CPU throttle | Frame time | p50 16.7 ms; p95 50-117 ms (one balanced run 483 ms) | >= 30 fps low-end phone | same | Misses the p95 target under this stand-in; **reference low-end phone not measured** |
| 2026-09-26 | A7 | same, 1x CPU | Main-thread cost of our rAF callback | simple p50 0.4-1.5 ms, p95 <= 3.4 ms; cinematic p50 1.0-2.3 ms, p95 <= 4.5 ms | 16.6 ms frame | rAF wrapper (ad hoc) | Style/layout/composite not included |
| 2026-09-26 | A7 | same, 1x CPU | Worst interaction (Event Timing, max duration) | 72-864 ms; the long ones are frames with no script time and no blocking time | INP < 200 ms | `scripts/profile.mjs frames` + Long Animation Frames | GPU/compositor stalls on this machine (seen with and without backdrop blur). Not attributed to app code; INP on phones **not measured** |
| 2026-09-26 | A7 | headless Chromium, SwiftShader (CI and local) | Draw calls per frame, city and story | simple 9, cinematic 19 | <= 60 low, <= 150 high | `tests/e2e/perf.spec.ts` (CI gate) | |
| 2026-09-26 | A7 | same | CLS, hero load to story | 0 (was 0.29: hero title re-wrapped when the display font swapped in) | 0 | `tests/e2e/perf.spec.ts` (CI gate) | Fix: all faces `font-display: optional` + preload; HUD actions fill the bar |
| 2026-09-26 | A7 | same | Hidden tab | 0 draw calls while hidden | stop when hidden | `tests/e2e/perf.spec.ts` (CI gate) | Low-rate rendering when nothing moves is **not** implemented (the scene always animates) |
| 2026-09-26 | A7 | Windows laptop, Chromium (Playwright), real GPU via ANGLE d3d11, 1440x900, cinematic | JS heap after forced GC, 10-minute explore soak | 3.0 MB start, 4.0 MB max; +0.4 MB after 1-minute warm-up (repeat, 5 min: +0.3 MB); DOM nodes and listeners flat | <= 300 MB, no growth | `scripts/profile.mjs soak` | Heap wanders +-0.2 MB minute to minute; a longer soak is needed to prove a plateau. GPU memory not measured |

Reference low-end device: budget Android class (Mali-G57 / Adreno 610, 4 GB), e.g. Galaxy A14 (PLAN section 6).
**Not measured yet (no hardware in this environment):** any phone, the reference low-end device, 4G first frame, INP.
Not measured in A0: frame rate, first frame, input latency, heap (nothing renders yet).
