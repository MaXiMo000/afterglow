...
| Security evidence (A8): rerunnable scripts (`mutation-spotcheck.py`, `sandbox-audit.sh`, `zap-scan.sh`, `load-test.sh`), results in `docs/SECURITY-EVIDENCE.md` | Evidence anyone can reproduce, not screenshots |
| Release (A8): tag-only workflow; images on GHCR pinned by digest; CycloneDX SBOMs; Sigstore build provenance | Supply-chain requirement T16 without new services |
| Privacy (A8): client pseudonym cleared by a DB trigger when a job ends; logs size-capped | Data minimisation enforced in one place, on every path |
