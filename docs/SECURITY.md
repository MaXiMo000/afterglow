...
| T20 | Client-side leakage | No tokens, no localStorage of anything sensitive (only UI prefs), no third-party requests (verify with a Playwright network assertion: only same-origin requests), no `console` dumping of data in prod | e2e | [partial] A3: e2e asserts only same-origin requests; no cookies, no localStorage/sessionStorage used at all; no console output of data. Fonts self-hosted |
