...
| T15 | Share-link / URL param abuse | Hash params parsed by a strict schema (numbers with ranges, enums); anything else ignored; never inserted into DOM as HTML | unit + fuzz | [done] A4-A5: share links `#v=1&r=owner/name&c=yaw,pitch,dist,x,z&t=..` parsed by a strict schema (known keys only, no duplicates, bounded decimal numbers, repo through the same parser as the API, 300-char cap); anything else ignored and no request is made (14 unit cases + e2e with hostile hashes); chapter hashes and `?quality=` are enums |
