...
| --- | --- | --- | --- | --- |
| T1 | SSRF / arbitrary clone target | Only `owner/name` accepted; server builds `https://github.com/<owner>/<name>.git` itself; regex + reserved-name checks; no redirects followed to other hosts; worker egress allowlist = github.com (network policy, not just code) | table-driven tests incl. unicode, `..`, `@`, `:`, `%`, whitespace, 10k chars | [partial] A1: strict `owner/name` parser (`core/repo.py`, table tests); URL built server-side; `http.followRedirects=false`; worker network is `internal` and its only exit is a CONNECT proxy allowing `github.com:443` to public IPs only (`egress/`, tests + live check). API side: A2 |
...
| T5 | Injection through repo-controlled strings (paths, names, messages) | Server: strip control chars/bidi overrides, cap lengths, normalise unicode, never build shell/SQL from them (argv arrays, parameterised queries). Client: `textContent` only, no `innerHTML`, no dynamic `href` except validated GitHub URL builder | fuzz + Playwright XSS payload fixtures | [partial] A1: server-side `clean_text` (NFC, control/zero-width/bidi chars replaced, length capped) and the result schema rejects any such character again; A0: client textContent only + bundle sink check. XSS payload e2e: A3 |
| T6 | Personal data leakage | Never return or log author emails; contributors pseudonymised by default; logs redact IPs after 24 h or hash them; no analytics; result cache contains no email | schema test asserts no `@`-emails in any response | [partial] A1: git is never asked for emails (`%an` only); authors become `Contributor N`; tests assert no names or email-shaped strings in results. Logs: A2 |
...
| T21 | WebGL/GPU hostile input | Cap instance counts from server data before allocating buffers; validate array lengths and finite numbers; handle context loss and shader-compile failure with the 2D fallback | tests | [partial] A1: result schema bounds every list and number and rejects NaN/out-of-range indices. Client-side checks: A3 |
